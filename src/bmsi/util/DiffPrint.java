...
public class DiffPrint {
  ...
  public static abstract class Base {
     ...
     * The default does nothing (except set output to system.out if not
     * otherwise set). Derived style classes can override to print the files
     * compared in the format for that style. */
    ...
    protected void print_number_range(char sepchar, int a, int b) {
      ...
      if (++b > ++a) outfile.print("" + a + sepchar + b);
      else outfile.print(b);
    }

    public static char change_letter(int inserts, int deletes) {
      if (inserts == 0) return 'd';
      else if (deletes == 0) return 'a';
      else return 'c';
    }
  }
  ...
  public static class NormalPrint extends Base {
    ...
    protected void print_hunk(Diff.change hunk) {
      ...
      if (deletes != 0) for (int i = first0; i <= last0; i++)
        print_1_line("< ", file0[i]);

      if (inserts != 0 && deletes != 0) outfile.println("---");

      /* Print the lines that the second file has. */
      if (inserts != 0) for (int i = first1; i <= last1; i++)
    ...
    protected void print_hunk(Diff.change hunk) {
      ...
      if (inserts != 0)
      {
        ...
        for (int i = first1; i <= last1; i++)
        {
          ...
          /* If the file's line is just a dot, it would confuse `ed'. So output
           * it with a double dot, and set the flag LEADING_DOT so that we will
           * output another ed-command later to change the double dot into a
           * single dot. */
  ...
  public static class ContextPrint extends Base {
    ...
    protected void print_context_label(String mark, File inf, String label) {
      setupOutput();
      if (label != null) outfile.println(mark + ' ' + label);
    ...
    protected void print_hunk(Diff.change hunk) {
      ...
      /* If we looked for and found a function this is part of, include its name
       * in the header of the diff section. */
      ...
      if (deletes != 0) {
        ...
        for (int i = first0; i <= last0; i++) {
          /* Skip past changes that apply (in file 0) only to lines before line
           * I. */
          ...
          /* The change NEXT covers this line. If lines were inserted here in
           * file 1, this is "changed". Otherwise it is "deleted". */
      ...
      if (inserts != 0) {
        ...
        for (int i = first1; i <= last1; i++) {
          /* Skip past changes that apply (in file 1) only to lines before line
           * I. */
          ...
          /* The change NEXT covers this line. If lines were deleted here in
           * file 0, this is "changed". Otherwise it is "inserted". */
  ...
  public static class UnifiedPrint extends ContextPrint {
    ...
    private void print_number_range(int a, int b) {
      ...
      /* Note: we can have B < A in the case of a range of no lines. In this
       * case, we should print the line number before the range, which is B. */
      if (b < a) outfile.print(b + ",0");
      else super.print_number_range(',', a, b);
    }
